...
class Message(BaseModel):
    role: str  # Made less strict for debugging - was Literal["system", "assistant", "user"]
    content: str
    
    @validator('role')
    def validate_role(cls, v):
        if v not in ["system", "assistant", "user"]:
            logger.warning(f"Invalid role: {v}, should be one of 'system', 'assistant', 'user'")
        return v
    
    @validator('content')
    def validate_content(cls, v):
        if not isinstance(v, str):
            logger.error(f"Content must be a string, got {type(v)}")
            raise ValueError(f"Content must be a string, got {type(v)}")
        return v
